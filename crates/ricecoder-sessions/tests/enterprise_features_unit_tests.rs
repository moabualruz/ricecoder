...
fn test_enterprise_sharing_policy_validation() {
    ...
    assert!(share_service
        .validate_enterprise_policy(&valid_policy)
        .is_ok());
    ...
    assert!(share_service
        .validate_enterprise_policy(&invalid_policy)
        .is_err());
}
...
fn test_enterprise_domain_restrictions() {
    ...
    assert_eq!(
        ...
        vec!["company.com".to_string(), "partner.org".to_string()]
    );
}
...
fn test_enterprise_share_revocation_with_audit() {
    ...
    // Revoke the share
    share_service
        .revoke_share(&share.id, Some("admin@company.com".to_string()))
        .unwrap();
...
fn test_enterprise_features_flag() {
    ...
    let audit_logger = create_audit_logger();
    let enterprise_service = ShareService::with_audit_logging(
        "https://enterprise.ricecoder.com".to_string(),
        audit_logger,
    );
...
fn test_enterprise_policy_edge_cases() {
    ...
    assert!(share_service
        .validate_enterprise_policy(&policy_no_limit)
        .is_ok());
    ...
    assert!(share_service
        .validate_enterprise_policy(&policy_max_expiration)
        .is_ok());
    ...
    assert!(share_service
        .validate_enterprise_policy(&policy_empty_domains)
        .is_ok());
}
...
fn test_enterprise_compliance_event_types() {
    ...
    let _imported = share_service
        .import_shared_session(
            &share.id,
            &shared_session,
            Some("importer@company.com".to_string()),
        )
        .unwrap();
...
fn test_enterprise_session_invalidation_cascade() {
    ...
    let removed = share_service
        .invalidate_session_shares(&session.id)
        .unwrap();
    assert_eq!(removed, 2);
