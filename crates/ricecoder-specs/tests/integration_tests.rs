...
fn integration_test_full_spec_loading_yaml_and_markdown() {
    ...
    manager
        .save_spec(&spec, &yaml_path)
        .expect("Failed to save YAML spec");
    assert!(yaml_path.exists(), "YAML spec file should exist");

    // Save as Markdown
    let md_path = temp_dir.path().join("spec.md");
    manager
        .save_spec(&spec, &md_path)
        .expect("Failed to save Markdown spec");
    assert!(md_path.exists(), "Markdown spec file should exist");

    // Load both specs
    let loaded_yaml = manager
        .load_spec(&yaml_path)
        .expect("Failed to load YAML spec");
    let loaded_md = manager
        .load_spec(&md_path)
        .expect("Failed to load Markdown spec");
    ...
    assert_eq!(
        loaded_yaml.requirements.len(),
        1,
        "YAML spec should have 1 requirement"
    );
    assert_eq!(
        loaded_md.requirements.len(),
        1,
        "Markdown spec should have 1 requirement"
    );
}
...
fn integration_test_governance_loading_and_merging() {
    ...
    let rule_1 = merged
        ...
        .iter()
        .find(|r| r.id == "rule-1")
        .expect("rule-1 should exist");
    assert_eq!(
        rule_1.description, "Project rule 1 (overrides global)",
        "Project rule should override global"
    );
    ...
    let rule_2 = merged
        ...
        .iter()
        .find(|r| r.id == "rule-2")
        .expect("rule-2 should exist");
    assert_eq!(
        rule_2.description, "Global rule 2",
    ...
    let rule_3 = merged
        ...
        .iter()
        .find(|r| r.id == "rule-3")
        .expect("rule-3 should exist");
    assert_eq!(
        rule_3.description, "Project rule 3",
        "Project rule should be included"
    );
}
...
fn integration_test_spec_discovery_and_filtering() {
    ...
    // Save specs
    manager
        .save_spec(&spec1, &temp_dir.path().join("feature-1.yaml"))
        .unwrap();
    manager
        .save_spec(&spec2, &temp_dir.path().join("feature-2.yaml"))
        .unwrap();
    manager
        .save_spec(&spec3, &temp_dir.path().join("feature-3.yaml"))
        .unwrap();

    // Discover all specs
    let discovered = manager
        .discover_specs(temp_dir.path())
        .expect("Failed to discover specs");
...
fn integration_test_hierarchical_spec_resolution() {
    ...
    let mut project_spec = create_test_spec(
        "project",
        "Project Spec",
        SpecPhase::Requirements,
        SpecStatus::Approved,
    ...
    let mut feature_spec = create_test_spec(
        "feature",
        "Feature Spec",
        SpecPhase::Design,
        SpecStatus::InReview,
    ...
    let resolved = SpecInheritanceResolver::resolve(&all_specs).expect("Resolution should succeed");
    ...
    let resolved_task = resolved
        ...
        .find(|s| s.id == "task")
        .cloned()
        .unwrap_or(task_spec.clone());

    // Verify resolved specs are sorted by precedence
    assert_eq!(resolved.len(), 3, "Should have 3 specs");
    assert_eq!(
        resolved[0].id, "project",
        "Project spec should be first (precedence 0)"
    );
    assert_eq!(
        resolved[1].id, "feature",
        "Feature spec should be second (precedence 1)"
    );
    assert_eq!(
        resolved[2].id, "task",
        "Task spec should be third (precedence 2)"
    );

    // Verify task spec has its own requirements
    assert_eq!(
        resolved_task.requirements.len(),
        1,
        "Task spec should have 1 requirement"
    );
    assert_eq!(
        resolved_task.requirements[0].id, "REQ-3",
        "Task spec should have REQ-3"
    );
}
...
fn integration_test_dependency_resolution_and_circular_detection() {
    ...
    let mut spec_a = create_test_spec(
        "spec-a",
        "Spec A",
        SpecPhase::Requirements,
        SpecStatus::Draft,
    ...
    let mut spec_b = create_test_spec(
        "spec-b",
        "Spec B",
        SpecPhase::Requirements,
        SpecStatus::Draft,
    ...
    let mut spec_c = create_test_spec(
        "spec-c",
        "Spec C",
        SpecPhase::Requirements,
        SpecStatus::Draft,
...
fn integration_test_change_tracking_and_history() {
    ...
    assert_eq!(
        history[0].rationale, "Initial creation",
        "Should preserve rationale"
    );
...
fn integration_test_ai_assisted_spec_writing_with_approval_gates() {
    ...
    let session = conversation_manager
        .create_session(session_id.to_string(), spec_id.to_string())
    ...
    let _msg1 = conversation_manager
        .add_message(
            session_id,
            "msg-1".to_string(),
            MessageRole::User,
    ...
    let _msg2 = conversation_manager
        .add_message(
            session_id,
            "msg-2".to_string(),
    ...
    let retrieved = conversation_manager
        .get_session(session_id)
        .expect("Should retrieve session");
    assert_eq!(
        retrieved.conversation_history.len(),
        2,
        "Should have 2 messages"
    );
    assert_eq!(
        retrieved.conversation_history[0].role,
        MessageRole::User,
        "First message should be from user"
    );
    assert_eq!(
        retrieved.conversation_history[1].role,
        MessageRole::Assistant,
        "Second message should be from assistant"
    );

    // Move to Requirements phase
    let updated_session = conversation_manager
        .update_phase(session_id, SpecPhase::Requirements)
        .expect("Should update phase");
    assert_eq!(
        updated_session.phase,
        SpecPhase::Requirements,
        "Phase should be updated to Requirements"
    );

    // Verify conversation history is preserved after phase update
    let final_session = conversation_manager
        .get_session(session_id)
        .expect("Should retrieve session");
    assert_eq!(
        final_session.conversation_history.len(),
        2,
    ...
    assert_eq!(
        gates[0].phase,
        SpecPhase::Discovery,
        "First gate should be Discovery"
    );
    assert_eq!(
        gates[1].phase,
        SpecPhase::Requirements,
        "Second gate should be Requirements"
    );
...
fn integration_test_spec_to_task_linking_and_traceability() {
    ...
    assert_eq!(
        tasks_for_req1.len(),
        1,
        "Requirement should be linked to 1 task"
    );
    assert_eq!(
        tasks_for_req1[0], "TASK-1",
        "Requirement should be linked to TASK-1"
    );
}
...
fn integration_test_governance_context_in_ai_prompts() {
    ...
    let project_rule = merged
        .rules
        .iter()
        .find(|r| r.id == "project-rule-1")
        .expect("Should find project rule");
    assert_eq!(
        project_rule.description, "Use async/await",
        "Project rule should be included"
    );

    // Verify global rule is included
    let global_rule = merged
        .rules
        .iter()
        .find(|r| r.id == "global-rule-1")
        .expect("Should find global rule");
    assert_eq!(
        global_rule.description, "Use Rust for core",
    ...
    assert!(
        prompt_context.contains("project-rule-1"),
        "Prompt should include project rule"
    );
    assert!(
        prompt_context.contains("global-rule-1"),
        "Prompt should include global rule"
    );
    assert!(
        prompt_context.contains("Use async/await"),
        "Prompt should include project rule description"
    );
    assert!(
        prompt_context.contains("Use Rust for core"),
        "Prompt should include global rule description"
    );
}
...
fn integration_test_complete_end_to_end_workflow() {
    ...
    manager
        ...
        .expect("Should save spec");

    // Step 2: Load the spec back
    let loaded_spec = manager.load_spec(&spec_path).expect("Should load spec");
    assert_eq!(
        loaded_spec.id, "auth-feature",
        "Loaded spec should have correct ID"
    );
    assert_eq!(
        loaded_spec.requirements.len(),
        2,
        "Loaded spec should have 2 requirements"
    );
    ...
    assert_eq!(
        task1_reqs.len(),
        1,
        "Task 1 should be linked to 1 requirement"
    );
    ...
    assert_eq!(
        task2_reqs.len(),
        1,
        "Task 2 should be linked to 1 requirement"
    );
    assert_eq!(task2_reqs[0], "REQ-2", "Task 2 should be linked to REQ-2");

    // Step 6: Verify complete workflow
    assert_eq!(
        spec_with_tasks.id, "auth-feature",
        "Spec ID should be preserved"
    );
    assert_eq!(
        spec_with_tasks.requirements.len(),
        2,
