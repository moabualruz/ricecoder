//! Property-based tests for steering context integration
...
// ============================================================================
// Property 10: Steering Context Integration
// ============================================================================

proptest! {
    /// Property: For any AI prompt generation, steering context from
    /// `projects/ricecoder/.ricecoder/steering/` SHALL be included with
    /// project steering taking precedence over global steering.
    ///
    /// This property verifies that:
    /// 1. Steering context is included in the generated prompt
    /// 2. Project steering takes precedence over global steering
    /// 3. All steering rules, standards, and templates are represented
    #[test]
    fn prop_steering_context_included_in_prompt(
        session in arb_spec_writing_session(),
        global_steering in arb_steering(),
        project_steering in arb_steering(),
        ...
        // Property 10.3: If steering has rules, they should be in the prompt
        if !project_steering.rules.is_empty() {
            prop_assert!(
                prompt.contains("Steering Rules"),
                "Prompt should include steering rules section when rules exist"
            );

            for rule in &project_steering.rules {
                prop_assert!(
        ...
        // Property 10.4: If steering has standards, they should be in the prompt
        if !project_steering.standards.is_empty() {
            prop_assert!(
                prompt.contains("Standards"),
                "Prompt should include standards section when standards exist"
            );

            for standard in &project_steering.standards {
                prop_assert!(
        ...
        // Property 10.5: If steering has templates, they should be in the prompt
        if !project_steering.templates.is_empty() {
            prop_assert!(
                prompt.contains("Available Templates"),
                "Prompt should include templates section when templates exist"
            );

            for template in &project_steering.templates {
                prop_assert!(
    ...
    /// Property: Project steering SHALL take precedence over global steering
    /// in the generated prompt.
    ///
    /// This property verifies that when both global and project steering
    /// define the same rule/standard/template, the project version appears
    /// in the prompt.
    #[test]
    fn prop_project_steering_takes_precedence(
        session in arb_spec_writing_session(),
        global_steering in arb_steering(),
        project_steering in arb_steering(),
        user_input in arb_user_input(),
        ...
        for project_rule in &project_steering.rules {
            prop_assert!(
            ...
            if let Some(global_rule) = global_steering.rules.iter().find(|r| r.id == project_rule.id) {
        ...
        for project_std in &project_steering.standards {
            prop_assert!(
        ...
        for project_tpl in &project_steering.templates {
            prop_assert!(
    ...
    /// Property: Steering context SHALL be consistent across multiple
    ...
    #[test]
    fn prop_steering_context_deterministic(
        session in arb_spec_writing_session(),
        global_steering in arb_steering(),
        project_steering in arb_steering(),
        user_input in arb_user_input(),
    ) {
        let result1 = AISpecWriter::build_prompt_with_steering_context(
            &session,
            &global_steering,
            &project_steering,
            &user_input,
        ...
        let result2 = AISpecWriter::build_prompt_with_steering_context(
            &session,
            &global_steering,
            &project_steering,
            &user_input,
        ...
        prop_assert_eq!(
            prompt1, prompt2,
            "Steering context generation should be deterministic"
        );
    }

    /// Property: Steering context SHALL include all rules, standards,
    /// and templates from the merged steering.
    ///
    /// This property verifies that no steering elements are lost
    /// during prompt generation.
    #[test]
    fn prop_steering_context_complete(
        session in arb_spec_writing_session(),
        global_steering in arb_steering(),
        project_steering in arb_steering(),
        user_input in arb_user_input(),
        ...
        for global_rule in &global_steering.rules {
            // Check if this rule is overridden by a project rule
            let is_overridden = project_steering.rules.iter().any(|r| r.id == global_rule.id);
        ...
        for global_std in &global_steering.standards {
            let is_overridden = project_steering.standards.iter().any(|s| s.id == global_std.id);
        ...
        for global_tpl in &global_steering.templates {
            let is_overridden = project_steering.templates.iter().any(|t| t.id == global_tpl.id);
    ...
    /// Property: Steering context SHALL be properly formatted for readability.
    ///
    /// This property verifies that the generated prompt is well-formatted
    /// and readable.
    #[test]
    fn prop_steering_context_well_formatted(
        session in arb_spec_writing_session(),
        global_steering in arb_steering(),
        project_steering in arb_steering(),
        user_input in arb_user_input(),
    ...
    /// Property: Steering context SHALL handle empty steering gracefully.
    ///
    /// This property verifies that the system works correctly when
    /// steering documents are empty.
    #[test]
    fn prop_steering_context_handles_empty_steering(
        session in arb_spec_writing_session(),
        user_input in arb_user_input(),
    ) {
        let empty_steering = Steering {
            rules: vec![],
            standards: vec![],
            templates: vec![],
        };
        ...
        prop_assert!(result.is_ok(), "Prompt building should succeed with empty steering");
        ...
        prop_assert!(
            prompt.contains("Phase Guidance"),
            "Prompt should include phase guidance even with empty steering"
        );

        // Property 10.21: Prompt should still contain user input
        prop_assert!(
            prompt.contains(&user_input),
            "Prompt should include user input even with empty steering"
        );
    }
}
