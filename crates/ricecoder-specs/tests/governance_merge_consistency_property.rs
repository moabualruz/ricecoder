//! Property-based tests for Governance merge consistency
//! **Feature: ricecoder-specs, Property 2: Governance Merge Consistency**
...
// ============================================================================
// Property 2: Governance Merge Consistency
// ============================================================================

proptest! {
    /// Property: For any global and project Governance documents, merging SHALL
    ...
    fn prop_merge_project_overrides_global(
        global in arb_governance(),
        project in arb_governance(),
    ) {
    ...
    /// Property: Merging empty project Governance SHALL preserve global Governance
    ///
    /// This property verifies that when project Governance is empty,
    /// the merged result is identical to global Governance.
    #[test]
    fn prop_merge_empty_project_preserves_global(
        global in arb_governance(),
    ) {
        let empty_project = Governance {
            rules: vec![],
            standards: vec![],
            templates: vec![],
        };

        let result = GovernanceLoader::merge(&global, &empty_project);
        prop_assert!(result.is_ok(), "Merge should succeed");
    ...
    /// Property: Merging empty global Governance SHALL preserve project Governance
    ///
    /// This property verifies that when global Governance is empty,
    /// the merged result is identical to project Governance.
    #[test]
    fn prop_merge_empty_global_preserves_project(
        project in arb_governance(),
    ) {
        let empty_global = Governance {
            rules: vec![],
            standards: vec![],
            templates: vec![],
        };

        let result = GovernanceLoader::merge(&empty_global, &project);
        prop_assert!(result.is_ok(), "Merge should succeed");
    ...
    /// Property: Merge SHALL be idempotent for identical Governance
    ///
    /// This property verifies that merging identical Governance documents
    /// produces the same result.
    #[test]
    fn prop_merge_idempotent_for_identical_governance(
        Governance in arb_governance(),
    ) {
        let result1 = GovernanceLoader::merge(&Governance, &Governance);
        prop_assert!(result1.is_ok(), "First merge should succeed");

        let merged1 = result1.unwrap();

        let result2 = GovernanceLoader::merge(&merged1, &Governance);
        prop_assert!(result2.is_ok(), "Second merge should succeed");

        let merged2 = result2.unwrap();

        // Property 2.16: Merging identical Governance should be idempotent
        prop_assert_eq!(
            merged1.rules.len(),
            merged2.rules.len(),
