...
use ricecoder_lsp::{LspServer, types::ServerState};
use serde_json::json;
...
async fn test_server_state_transitions() {
    ...
    let init_result = server.handle_initialize(json!({
        "capabilities": {}
    })).await;
...
async fn test_document_lifecycle() {
    ...
    // Open document
    let open_result = server.handle_did_open(json!({
        "textDocument": {
            "uri": uri,
            "text": content
        }
    ...
    let change_result = server.handle_did_change(json!({
    ...
    // Close document
    let close_result = server.handle_did_close(json!({
        "textDocument": { "uri": uri }
    })).await;
...
async fn test_multiple_documents() {
    ...
    // Open first document
    let open1_result = server.handle_did_open(json!({
        "textDocument": {
            "uri": uri1,
            "text": content1
        }
    ...
    let open2_result = server.handle_did_open(json!({
        "textDocument": {
            "uri": uri2,
            "text": content2
        }
    ...
    let close1_result = server.handle_did_close(json!({
    ...
    let close2_result = server.handle_did_close(json!({
...
async fn test_error_handling_invalid_state() {
    ...
    let hover_result = server.handle_hover(json!({
        "textDocument": { "uri": "file://test.rs" },
        "position": { "line": 0, "character": 0 }
    })).await;
    assert!(hover_result.is_err());
    
    let diagnostics_result = server.handle_diagnostics(json!({
        "textDocument": { "uri": "file://test.rs" }
    })).await;
...
async fn test_server_capabilities_response() {
    ...
    assert_eq!(server_info.get("name").and_then(|v| v.as_str()), Some("ricecoder-lsp"));
    assert_eq!(server_info.get("version").and_then(|v| v.as_str()), Some("0.1.0"));
