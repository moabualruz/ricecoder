...
use ricecoder_completion::{
    ...
    CompletionResult, GenericCompletionEngine, Position as CompletionPosition, ProviderRegistry,
    PythonCompletionProvider, RustCompletionProvider, TreeSitterContextAnalyzer,
...
impl CompletionGenerator for MockCompletionGenerator {
    async fn generate_completions(
        ...
        Ok(vec![
            CompletionItem::new(
                "let".to_string(),
                ricecoder_completion::CompletionItemKind::Keyword,
                "let".to_string(),
            ),
            CompletionItem::new(
                "fn".to_string(),
...
async fn test_completion_disabled() {
    ...
    assert!(
        items.is_empty(),
        "Should return empty completions when disabled"
    );
}
...
async fn test_completion_item_resolve() {
    ...
    assert_eq!(
        resolved.get("label").and_then(|l| l.as_str()),
        Some("test_function")
    );
    assert_eq!(
        resolved.get("resolved").and_then(|r| r.as_bool()),
        Some(true)
    );
}
...
async fn test_completion_memory_under_load() {
    ...
    for _ in 0..100 {
        ...
        assert!(
            result.is_ok(),
            "Completion request should succeed under load"
        );
    }
}
