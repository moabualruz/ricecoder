...
use ricecoder_storage::{PathResolver, get_builtin_language_configs, get_language_config};
...
fn test_builtin_configs_have_required_fields() {
    ...
    for (lang, config_str) in configs {
        let value: serde_yaml::Value = serde_yaml::from_str(config_str)
            .expect(&format!("Failed to parse {} config", lang));
        
        // Should have language field
        assert!(value["language"].is_string(), "{} missing language field", lang);
        
        // Should have file_extensions field
        assert!(value["file_extensions"].is_sequence(), "{} missing file_extensions field", lang);
    }
}
