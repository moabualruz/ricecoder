...
async fn test_completion_generation_time_target() {
    ...
    println!("Completion generation time: {}ms (target: <{}ms)", elapsed_ms, target_ms);
    assert!(
...
async fn test_completion_generation_time_average() {
    ...
    println!("Average completion generation time: {}ms (target: <{}ms)", average_ms, target_ms);
    assert!(
...
async fn test_prefix_filtering_performance() {
    ...
    println!("Prefix filtering time: {}ms (target: <{}ms)", elapsed_ms, target_ms);
    assert!(
        elapsed_ms < target_ms,
...
async fn test_completion_with_large_code() {
    ...
    println!("Completion with large code ({}KB): {}ms (target: <{}ms)", 
             code.len() / 1024, elapsed_ms, target_ms);
    assert!(
        elapsed_ms < target_ms,
...
async fn test_rapid_completion_requests_performance() {
    ...
    println!("Rapid requests average: {}ms per request (target: <{}ms)", average_ms, target_ms);
    assert!(
        average_ms < target_ms,
