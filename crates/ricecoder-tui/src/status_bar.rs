...
mod tests {
    ...
    fn test_left_section() {
        ...
        let spans = status_bar.left_section();
        assert!(!spans.is_empty());
        // Check that provider@model is included
        assert!(spans.iter().any(|s| s.content.contains("TestProvider@test-model")));
        // Check that connection status is included
        assert!(spans.iter().any(|s| s.content.contains("✓")));
        // Check that session info is included
        assert!(spans.iter().any(|s| s.content.contains("Test Session (10)")));
    }

    #[test]
    fn test_center_section() {
        let status_bar = StatusBarWidget::new()
            .with_project_name(Some("test-project".to_string()))
            .with_working_directory(PathBuf::from("/home/user/test"))
            .with_git_branch(Some("develop".to_string()))
            .with_token_usage(Some(TokenUsage { used: 1000, limit: Some(4000) }));

        let spans = status_bar.center_section();
        assert!(!spans.is_empty());
        // Check project name
        assert!(spans.iter().any(|s| s.content.contains("[test-project]")));
        // Check working directory
        assert!(spans.iter().any(|s| s.content.contains("/home/user/test")));
        // Check git branch
        assert!(spans.iter().any(|s| s.content.contains("(develop)")));
        // Check token usage
        assert!(spans.iter().any(|s| s.content.contains("Tokens: 1000/4000")));
    }
    ...
    fn test_right_section() {
        let status_bar = StatusBarWidget::new()
            .with_input_mode(InputMode::Normal)
            .with_recording_status(Some("macro1".to_string()))
            .with_search_status(Some("pattern".to_string()))
            .with_selection_status(Some("5 lines".to_string()));

        let spans = status_bar.right_section();
        assert!(!spans.is_empty());
        // Check input mode
        assert!(spans.iter().any(|s| s.content.contains("NORMAL")));
        // Check recording status
        assert!(spans.iter().any(|s| s.content.contains("REC:macro1")));
        // Check search status
        assert!(spans.iter().any(|s| s.content.contains("SEARCH:pattern")));
        // Check selection status
        assert!(spans.iter().any(|s| s.content.contains("SEL:5 lines")));
    }
}