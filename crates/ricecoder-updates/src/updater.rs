...
impl BinaryUpdater {
    ...
    pub async fn install_update(&self, release_info: &ReleaseInfo) -> Result<UpdateOperation> {
        ...
        operation.security_validation =
            match self.validate_security(&download_path, release_info).await {
    ...
    async fn validate_policy_for_update(&self, release_info: &ReleaseInfo) -> Result<()> {
        ...
        let download_size_mb = release_info
            .downloads
            .values()
        ...
        match self
            .policy
            .evaluate_update(&release_info.channel, download_size_mb, &compliance_tags)
        {
            crate::policy::PolicyResult::Allowed => Ok(()),
            crate::policy::PolicyResult::Denied(reason) => {
                Err(UpdateError::policy_violation(reason))
            }
            crate::policy::PolicyResult::RequiresApproval => Err(UpdateError::policy_violation(
                "Update requires manual approval",
            )),
        }
    }
    ...
    async fn create_backup(&self) -> Result<PathBuf> {
        ...
        self.copy_directory_recursive(&self.install_path, &backup_path)
            .await?;
    ...
    async fn download_update(&self, release_info: &ReleaseInfo) -> Result<PathBuf> {
        ...
        let download_info = release_info.downloads.get(&platform_key).ok_or_else(|| {
    ...
    async fn validate_security(
        ...
        let platform_key = self.get_platform_key();
        let download_info = release_info
            .downloads
            .get(&platform_key)
        ...
        let checksum_valid = self
            .validate_checksum(file_path, &download_info.sha256)
            .await?;
        ...
        let signature_valid = if self.policy.signature_required() {
            if let Some(signature) = &download_info.signature {
                ...
                return Err(UpdateError::security_validation(
    ...
    async fn perform_installation(&self, download_path: &Path) -> Result<()> {
        ...
        // Update version file
        self.update_version_file(&semver::Version::new(0, 1, 0))
            .await?; // Would be from release_info
    ...
    async fn rollback_to_backup(&self, backup_path: &Path) -> Result<()> {
        ...
        self.copy_directory_recursive(backup_path, &self.install_path)
            .await?;
    ...
    fn get_version_from_binary(&self) -> Result<semver::Version> {
        ...
        if let Some(version_part) = version_line.split_whitespace().nth(1) {
            ...
            Err(UpdateError::generic(
                "Could not extract version from binary output",
            ))
        }
    }
