...
use ricecoder_providers::{
    ...
    provider::{ProviderRegistry, ProviderManager, ChatStream},
...
impl Provider for FailThenSucceedProvider {
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse, ProviderError> {
        ...
        if count == 0 {
            Err(ProviderError::ProviderError("Temporary failure".to_string()))
        } else {
            Ok(ChatResponse {
...
async fn test_fallback_chain_multiple_providers() {
    ...
    let result = manager.chat_with_provider(&provider1, request.clone()).await;
    assert!(result.is_err());
    
    // Provider 2 should fail
    let result = manager.chat_with_provider(&provider2, request.clone()).await;
    assert!(result.is_err());
    
    // Provider 3 should succeed
    let result = manager.chat_with_provider(&provider3, request.clone()).await;
    assert!(result.is_ok());
...
async fn test_retry_logic_with_fail_then_succeed() {
    ...
    let manager = ProviderManager::new(registry, "retry-provider".to_string())
        .with_retry_count(3);
