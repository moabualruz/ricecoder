...
fn test_google_model_info_gemini_1_5_pro() {
    ...
    assert!(gemini
        .capabilities
        .contains(&ricecoder_providers::Capability::Vision));
}
...
fn test_google_token_counting_special_characters() {
    ...
    let tokens_special = provider
        .count_tokens(with_special, "gemini-1.5-pro")
        .unwrap();
...
fn test_google_token_counting_all_models() {
    ...
    for model in provider.models() {
        ...
        assert!(
            tokens.is_ok(),
            "Token counting should work for model {}",
            model.id
        );
        assert!(
            tokens.unwrap() > 0,
            "Token count should be positive for model {}",
            model.id
        );
    }
}
