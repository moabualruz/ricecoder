...
use ricecoder_providers::{Capability, ChatRequest, Provider, ZenProvider};
...
fn test_zen_models_pricing_values() {
    ...
    for model in models {
        if let Some(pricing) = model.pricing {
            assert!(
                pricing.input_per_1k_tokens > 0.0,
                "Input pricing should be positive"
            );
            assert!(
                pricing.output_per_1k_tokens > 0.0,
                "Output pricing should be positive"
            );
        }
    }
}
