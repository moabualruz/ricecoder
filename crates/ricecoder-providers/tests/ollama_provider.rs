...
async fn test_ollama_connection_error_handling() {
    ...
    let provider = OllamaProvider::new("http://invalid-host-that-does-not-exist:11434".to_string()).unwrap();
...
async fn test_ollama_full_chat_flow_integration() {
    ...
    let _mock_tags = server.mock("GET", "/api/tags")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(r#"{
            "models": [
    ...
    let _mock_chat = server.mock("POST", "/api/chat")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(r#"{
...
async fn test_ollama_model_pull_integration() {
    ...
    let _mock_pull = server.mock("POST", "/api/pull")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(r#"{
            "status": "success",
...
async fn test_ollama_model_remove_integration() {
    ...
    let _mock_delete = server.mock("DELETE", "/api/delete")
        .with_status(200)
        .with_header("content-type", "application/json")
...
async fn test_ollama_health_check_integration() {
    ...
    // Mock the /api/tags endpoint for health check
    let _mock_health = server.mock("GET", "/api/tags")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(r#"{"models": []}"#)
        .create_async()
...
async fn test_ollama_error_recovery_integration() {
    ...
    // Mock the /api/chat endpoint to fail first, then succeed
    let _mock_chat_fail = server.mock("POST", "/api/chat")
        .with_status(500)
        .with_header("content-type", "application/json")
        .with_body(r#"{"error": "internal server error"}"#)
        .expect_at_least(1)
...
async fn test_ollama_concurrent_operations_integration() {
    ...
    let provider = std::sync::Arc::new(
        OllamaProvider::new(base_url).unwrap()
    );
...
{"model": "mistral", "message": {"role": "assistant", "content": "!"}, "done": true}"#)
    ...
    assert_eq!(responses[2].as_ref().unwrap().finish_reason, FinishReason::Stop);
}
...
async fn test_ollama_streaming_multiple_chunks() {
    ...
    assert_eq!(responses[9].as_ref().unwrap().finish_reason, FinishReason::Stop);
}
