...
async fn run_security_scan(path: PathBuf, scan_type: String) -> anyhow::Result<()> {
    ...
    match scan_type.as_str() {
        "dependencies" | "all" => {
            ...
            println!("Dependency scan completed. Found {} vulnerabilities", result.vulnerabilities.len());
        }
        "code" | "all" => {
            let result = scanner.scan_code(&path).await?;
            println!("Code scan completed. Found {} issues in {} files", result.issues.len(), result.files_scanned);
        }
        "config" | "all" => {
            let result = scanner.scan_config(&path).await?;
            println!("Config scan completed. Found {} issues in {} files", result.issues.len(), result.files_scanned);
        }
        "licenses" | "all" => {
            let result = scanner.scan_licenses(&path).await?;
            println!("License scan completed. Found {} incompatible licenses", result.incompatible_licenses.len());
        }
        _ => {
...
async fn run_penetration_test(url: String) -> anyhow::Result<()> {
    ...
    println!("Penetration test completed. Found {} vulnerabilities", 
             results.iter().map(|r| r.vulnerabilities_found.len()).sum::<usize>());
    
    for result in results {
        if !result.vulnerabilities_found.is_empty() {
            println!("Test: {} - Found {} vulnerabilities", result.test_name, result.vulnerabilities_found.len());
        }
    }
...
async fn run_compliance_check(standard: String) -> anyhow::Result<()> {
    ...
    match standard.as_str() {
        "soc2" | "all" => {
            ...
            println!("SOC 2 compliance: {:.1}% ({})", result.score, if result.passed { "PASSED" } else { "FAILED" });
        }
        "gdpr" | "all" => {
            let result = checker.check_gdpr_compliance().await?;
            println!("GDPR compliance: {:.1}% ({})", result.score, if result.passed { "PASSED" } else { "FAILED" });
        }
        "hipaa" | "all" => {
            let result = checker.check_hipaa_compliance().await?;
            println!("HIPAA compliance: {:.1}% ({})", result.score, if result.passed { "PASSED" } else { "FAILED" });
        }
        "owasp" | "all" => {
            let result = checker.check_owasp_compliance().await?;
            println!("OWASP compliance: {:.1}% ({})", result.score, if result.passed { "PASSED" } else { "FAILED" });
        }
        _ => {
    ...
    match test_type.as_str() {
        "input" | "all" => {
            ...
            for input in malicious_inputs {
                ...
                println!("Input validation for '{}': {}", input, if result.is_safe { "SAFE" } else { "BLOCKED" });
            }
        }
        "auth" | "all" => {
            let result = validator.validate_authentication("user", "password").await?;
            println!("Authentication test: {}", if result.success { "PASSED" } else { "FAILED" });
        }
        "encryption" | "all" => {
            let result = validator.encrypt_data("sensitive data").await?;
            println!("Encryption test: {}", if result.success { "PASSED" } else { "FAILED" });
        }
        _ => {
