...
pub async fn execute_tool(
    ...
    if request.tool_name.len() > 100 {
        return Err(ApiError::BadRequest("Tool name too long (max 100 characters)".to_string()));
    }
    if !request.tool_name.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-') {
        return Err(ApiError::BadRequest("Tool name can only contain alphanumeric characters, underscores, and hyphens".to_string()));
    }

    // Validate parameters
    if let Some(timeout) = request.timeout_seconds {
        if timeout > 300 { // Max 5 minutes
            return Err(ApiError::BadRequest("Timeout too long (max 300 seconds)".to_string()));
        }
    }
    ...
    let parameters = if let serde_json::Value::Object(map) = request.parameters {
        ...
        for (key, value) in &map {
            if key.len() > 100 {
                return Err(ApiError::BadRequest(format!("Parameter name '{}' too long", key)));
            }
            let value_str = serde_json::to_string(value)?;
            if value_str.len() > 10000 {
                return Err(ApiError::BadRequest(format!("Parameter '{}' value too large", key)));
            }
        }
        map.into_iter().collect()
    } else {
        return Err(ApiError::BadRequest("Parameters must be a JSON object".to_string()));
    };
