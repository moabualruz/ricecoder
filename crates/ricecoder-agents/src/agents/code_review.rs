...
impl CodeReviewAgent {
    ...
    fn analyze_code_quality(&self, code: &str) -> Vec<Finding> {
        ...
        if self.has_structural_issues(code) {
            findings.push(Finding {
                ...
                location: None,
                suggestion: Some("Consider breaking down complex functions or reducing nesting depth".to_string()),
            });
        }
    ...
    fn scan_security(&self, code: &str) -> Vec<Finding> {
        ...
        if self.has_hardcoded_secrets(code) {
            findings.push(Finding {
                ...
                location: None,
                suggestion: Some("Move secrets to environment variables or configuration files".to_string()),
            });
        }
        ...
        if self.has_unsafe_operations(code) {
            findings.push(Finding {
                ...
                location: None,
                suggestion: Some("Ensure unsafe code has proper safety documentation and justification".to_string()),
            });
        }
    ...
    fn analyze_performance(&self, code: &str) -> Vec<Finding> {
        ...
        if self.has_unnecessary_allocations(code) {
            findings.push(Finding {
                ...
                location: None,
                suggestion: Some("Consider using stack allocation or references instead".to_string()),
            });
        }
    ...
    fn has_complexity_issues(&self, code: &str) -> bool {
        ...
        let complexity = if_count + else_count + match_count + for_count + while_count + and_count + or_count;
        ...
        if code.contains("read_line") {
            ...
            for (i, line) in lines.iter().enumerate() {
                if line.contains("read_line") {
                    ...
                    for next_line in lines.iter().skip(i + 1).take(std::cmp::min(5, lines.len() - i - 1)) {
                        if next_line.contains("trim") || next_line.contains("parse") || 
                           next_line.contains("validate") || next_line.contains("check") {
                            has_validation = true;
                            break;
                        }
                    }
...
mod tests {
    ...
    fn test_code_review_agent_with_custom_config() {
        ...
        settings.insert("enable_quality_checks".to_string(), serde_json::json!(false));
    ...
    fn test_code_review_agent_config_schema() {
        ...
        assert!(schema.properties.contains_key("enable_best_practice_checks"));
    ...
    fn test_analyze_performance() {
        let agent = CodeReviewAgent::new();
        let findings = agent.analyze_performance("for i in 0..10 { for j in 0..10 { for k in 0..10 {} } }");
