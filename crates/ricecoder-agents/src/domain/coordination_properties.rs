...
mod tests {
    ...
    fn create_recommendation(domain: &str, category: &str, technologies: Vec<&str>) -> Recommendation {
        Recommendation {
    ...
    fn property_full_stack_coordination_all_domains_present() {
        ...
        for _ in 0..5 {
            let coordination = coordinator.coordinate_full_stack(recommendations.clone()).unwrap();

            // Property: All three domains should be present
            assert!(!coordination.web_recommendations.is_empty(), "Web recommendations should be present");
            assert!(!coordination.backend_recommendations.is_empty(), "Backend recommendations should be present");
            assert!(!coordination.devops_recommendations.is_empty(), "DevOps recommendations should be present");
            ...
            assert_eq!(
                coordination.total_recommendations,
                6,
                "Should have all 6 recommendations"
            );

            // Property: Each domain should have correct number of recommendations
            assert_eq!(coordination.web_recommendations.len(), 2, "Web should have 2 recommendations");
            assert_eq!(coordination.backend_recommendations.len(), 2, "Backend should have 2 recommendations");
            assert_eq!(coordination.devops_recommendations.len(), 2, "DevOps should have 2 recommendations");
        }
    }
    ...
    fn property_full_stack_coordination_partial_stack() {
        ...
        for _ in 0..5 {
            ...
            assert!(!coordination.is_full_stack, "Should not be marked as full-stack");
            ...
            assert!(coordination.devops_recommendations.is_empty(), "DevOps recommendations should be empty");

            // Property: Total should be 2
            assert_eq!(coordination.total_recommendations, 2, "Should have 2 recommendations");
        }
    }
    ...
    fn property_full_stack_coordination_tech_stack_consistency() {
        ...
        for _ in 0..5 {
            ...
            assert!(!coordination.web_recommendations.is_empty(), "Web should have recommendations");
            assert!(!coordination.backend_recommendations.is_empty(), "Backend should have recommendations");
            assert!(!coordination.devops_recommendations.is_empty(), "DevOps should have recommendations");

            // Property: Each recommendation should have technologies
            for rec in &coordination.web_recommendations {
                assert!(!rec.technologies.is_empty(), "Web recommendations should have technologies");
            }
            for rec in &coordination.backend_recommendations {
                assert!(!rec.technologies.is_empty(), "Backend recommendations should have technologies");
            }
            for rec in &coordination.devops_recommendations {
                assert!(!rec.technologies.is_empty(), "DevOps recommendations should have technologies");
            }
        }
    }
    ...
    fn property_full_stack_coordination_empty_input() {
        ...
        for _ in 0..5 {
            ...
            assert!(!coordination.is_full_stack, "Empty input should not be full-stack");
            assert_eq!(coordination.total_recommendations, 0, "Should have no recommendations");
            assert!(coordination.web_recommendations.is_empty(), "Web should be empty");
            assert!(coordination.backend_recommendations.is_empty(), "Backend should be empty");
            assert!(coordination.devops_recommendations.is_empty(), "DevOps should be empty");
        }
    }
    ...
    fn property_full_stack_coordination_single_domain() {
        ...
        for domain in domains {
            ...
            for _ in 0..3 {
                ...
                assert!(!coordination.is_full_stack, "Single domain should not be full-stack");
    ...
    fn property_full_stack_coordination_multiple_per_domain() {
        ...
        for _ in 0..3 {
            ...
            assert_eq!(coordination.total_recommendations, 9, "All 9 recommendations should be preserved");
            assert_eq!(coordination.web_recommendations.len(), 3, "Web should have 3 recommendations");
            assert_eq!(coordination.backend_recommendations.len(), 3, "Backend should have 3 recommendations");
            assert_eq!(coordination.devops_recommendations.len(), 3, "DevOps should have 3 recommendations");
    ...
    fn property_full_stack_coordination_recommendation_integrity() {
        ...
        for _ in 0..3 {
            ...
            assert!(coordination.web_recommendations[0].technologies.contains(&"React".to_string()));
            assert!(coordination.web_recommendations[0].technologies.contains(&"TypeScript".to_string()));
            ...
            assert_eq!(coordination.backend_recommendations[0].technologies.len(), 1);
            assert!(coordination.backend_recommendations[0].technologies.contains(&"REST".to_string()));

            // Property: DevOps recommendation should be intact
            assert_eq!(coordination.devops_recommendations[0].domain, "devops");
            assert_eq!(coordination.devops_recommendations[0].category, "containerization");
            assert_eq!(coordination.devops_recommendations[0].technologies.len(), 1);
            assert!(coordination.devops_recommendations[0].technologies.contains(&"Docker".to_string()));
        }
    }
}
