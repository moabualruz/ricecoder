use crate::models::{ComplexityLevel, ThinkingDepth, ThinkMoreConfig};
...
impl ThinkMoreController {
    ...
    pub fn enable(&self) -> Result<()> {
        let mut config = self.global_config.lock()
            .map_err(|_| ModeError::ConfigError("Failed to acquire lock on global config".to_string()))?;
        config.enabled = true;
        Ok(())
    }
    ...
    pub fn set_task_config(&self, task_id: String, config: ThinkMoreConfig) -> Result<()> {
        let mut task_configs = self.task_configs.lock()
            .map_err(|_| ModeError::ConfigError("Failed to acquire lock on task configs".to_string()))?;
    ...
    pub fn get_task_config(&self, task_id: &str) -> Result<Option<ThinkMoreConfig>> {
        let task_configs = self.task_configs.lock()
            .map_err(|_| ModeError::ConfigError("Failed to acquire lock on task configs".to_string()))?;
    ...
    pub fn start_thinking(&self, depth: ThinkingDepth) -> Result<()> {
        let mut state = self.thinking_state.lock()
            .map_err(|_| ModeError::ConfigError("Failed to acquire lock on thinking state".to_string()))?;
        state.active = true;
    ...
    pub fn is_thinking(&self) -> Result<bool> {
        let state = self.thinking_state.lock()
            .map_err(|_| ModeError::ConfigError("Failed to acquire lock on thinking state".to_string()))?;
        Ok(state.active)
    }
...
mod tests {
    ...
    fn test_task_config() {
        ...
        controller.set_task_config("task1".to_string(), config.clone()).unwrap();
    ...
    fn test_should_auto_enable() {
        ...
        assert!(controller.should_auto_enable(ComplexityLevel::Complex).unwrap());
        assert!(!controller.should_auto_enable(ComplexityLevel::Moderate).unwrap());
        assert!(!controller.should_auto_enable(ComplexityLevel::Simple).unwrap());
    }
