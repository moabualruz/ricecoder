...
impl TaskConfigManager {
    ...
    pub fn add_task_context(&self, task_id: &str, key: String, value: serde_json::Value) -> Result<()> {
        let mut configs = self.configs.lock()
            .map_err(|_| ModeError::ConfigError("Failed to acquire lock on task configs".to_string()))?;
    ...
    pub fn get_task_context(&self, task_id: &str, key: &str) -> Result<Option<serde_json::Value>> {
        ...
        Ok(configs.get(task_id).and_then(|tc| tc.get_context(key).cloned()))
    }

    /// Remove context data from a task
    pub fn remove_task_context(&self, task_id: &str, key: &str) -> Result<Option<serde_json::Value>> {
        let mut configs = self.configs.lock()
            .map_err(|_| ModeError::ConfigError("Failed to acquire lock on task configs".to_string()))?;
...
mod tests {
    ...
    fn test_manager_get_task_config() {
        ...
        manager.register_task("task1".to_string(), config.clone()).unwrap();
    ...
    fn test_manager_add_task_context() {
        ...
        manager.register_task("task1".to_string(), config).unwrap();
        manager.add_task_context("task1", "key1".to_string(), serde_json::json!("value1")).unwrap();
    ...
    fn test_manager_clear_task_context() {
        ...
        manager.register_task("task1".to_string(), config).unwrap();
        manager.add_task_context("task1", "key1".to_string(), serde_json::json!("value1")).unwrap();
        manager.add_task_context("task1", "key2".to_string(), serde_json::json!("value2")).unwrap();
    ...
    fn test_manager_get_all_task_ids() {
        ...
        manager.register_task("task1".to_string(), config.clone()).unwrap();
        manager.register_task("task2".to_string(), config.clone()).unwrap();
