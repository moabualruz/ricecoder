...
//!
//! Aggregates represent clusters of domain objects that can be treated as a single unit.
...
impl FileEdit {
    /// Create a new FileEdit with validation
    pub fn new(
        file_path: FilePath,
        old_pattern: EditPattern,
        new_content: String,
    ...
    /// Validate pattern exists in file content (pure function)
    pub fn validate_pattern_exists(&mut self, file_content: &str) -> DomainResult<()> {
        let found = file_content.contains(self.old_pattern.pattern());
        if !found {
            return Err(DomainError::InvalidFileEdit(
                format!("Pattern '{}' not found", self.old_pattern.pattern())
            ));
        }
        Ok(())
    }
...
mod tests {
    use super::*;
    use crate::domain::value_objects::EditPattern;
    ...
    fn test_file_edit_creation() {
        ...
        assert!(!edit.is_dry_run());
        assert_eq!(edit.events().len(), 1); // FileEditValidated event
    }
    
    #[test]
    fn test_file_edit_empty_content_fails() {
        let file_path = FilePath::new("src/main.rs").unwrap();
        let pattern = EditPattern::literal("hello").unwrap();
        
        let result = FileEdit::new(file_path, pattern, "   ".to_string(), false);
        assert!(result.is_err());
    }
    
    #[test]
    fn test_file_edit_pattern_validation() {
        let file_path = FilePath::new("src/main.rs").unwrap();
        let pattern = EditPattern::literal("hello").unwrap();
        
        let mut edit = FileEdit::new(file_path, pattern, "hi".to_string(), false).unwrap();
        
        // Test pure pattern validation logic (no file I/O)
        let result = edit.validate_pattern_exists("hello world");
        assert!(result.is_ok());
        
        let result = edit.validate_pattern_exists("goodbye world");
        assert!(result.is_err());
    }
    
    #[test]
    fn test_file_edit_mark_executed() {
        let file_path = FilePath::new("src/main.rs").unwrap();
        let pattern = EditPattern::literal("hello").unwrap();
        
        let mut edit = FileEdit::new(file_path, pattern, "hi".to_string(), false).unwrap();
        edit.mark_executed(1);
        
        assert_eq!(edit.events().len(), 2); // Validated + Executed events
    }
    ...
    fn test_search_result_creation() {
        ...
        let match1 = SearchMatch::new(1, 0, "hello".to_string());
        let matches = vec![match1];
        
        let result = SearchResult::new(file_path, matches);
        assert_eq!(result.total_matches(), 1);
        assert!(result.has_matches());
        assert_eq!(result.events().len(), 1); // SearchExecuted event
    }
    
    #[test]
    fn test_search_result_no_matches() {
        let file_path = FilePath::new("src/main.rs").unwrap();
        let result = SearchResult::new(file_path, vec![]);
