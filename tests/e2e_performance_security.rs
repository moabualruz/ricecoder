...
async fn run_baseline_performance_tests(performance_monitor: &Arc<PerformanceMonitor>) {
    ...
    performance_monitor
        .record_metric("cli_command_latency", cli_performance.as_millis() as f64)
        .await;
    ...
    performance_monitor
        .record_metric(
            ...
            session_performance.as_millis() as f64,
        )
        .await;
    ...
    performance_monitor
        .record_metric(
            "provider_switch_latency",
            provider_performance.as_millis() as f64,
        )
        .await;
    ...
    performance_monitor
        .record_metric("concurrent_ops_throughput", concurrent_performance)
        .await;
...
async fn test_session_management_performance() -> Duration {
    ...
    for i in 0..10 {
        ...
        session_manager
            .add_message_to_session(&session_id, "Performance test message")
            .await
...
async fn test_provider_switching_performance() -> Duration {
    let provider_manager = ricecoder_providers::ProviderManager::new()
        .await
    ...
    provider_manager
        .add_provider("perf-openai", config1)
        .await
        .expect("Failed to add provider");
    provider_manager
        .add_provider("perf-anthropic", config2)
        .await
        .expect("Failed to add provider");
    ...
    for _ in 0..20 {
        provider_manager
            .set_active_provider("perf-openai")
            .await
            .expect("Failed to switch provider");
        provider_manager
            .set_active_provider("perf-anthropic")
            .await
            .expect("Failed to switch provider");
    }
...
async fn test_concurrent_operations_performance() -> f64 {
    ...
    for i in 0..20 {
        let handle = tokio::spawn(async move {
            ...
            session_manager
                .add_message_to_session(&session_id, "Concurrent message")
                .await
                .expect("Failed to add message");
...
async fn test_input_validation_security(security_validator: &Arc<SecurityValidator>) {
    ...
    for input in malicious_inputs {
        ...
        assert!(
            ...
            "Malicious input should be flagged as unsafe: {}",
            input
        );

        security_validator
            .record_validation_attempt(input, validation_result.is_safe)
            .await;
    }
...
async fn test_auth_security(security_validator: &Arc<SecurityValidator>) {
    ...
    for (username, password, should_succeed) in auth_attempts {
        let auth_result = security_validator
            .validate_authentication(username, password)
            .await;
        assert_eq!(
            auth_result.success, should_succeed,
            "Auth validation failed for user: {}, expected: {}",
    ...
    for (user, permission, should_allow) in authz_checks {
        let authz_result = security_validator
            .validate_authorization(user, permission)
            .await;
        assert_eq!(
            authz_result.allowed, should_allow,
            "Authz validation failed for user: {}, permission: {}",
            user, permission
        );
    }
}
...
async fn test_encryption_security(security_validator: &Arc<SecurityValidator>) {
    ...
    // Test encryption
    let encrypted = security_validator
    ...
    // Test decryption
    let decrypted = security_validator
    ...
    let still_decrypted = security_validator
        .decrypt_data(&encrypted)
        .await
        .expect("Failed to decrypt with rotated keys");
    assert_eq!(
        still_decrypted, sensitive_data,
        "Should decrypt with rotated keys"
    );
}
...
async fn test_api_security(security_validator: &Arc<SecurityValidator>) {
    ...
    for (api_key, should_be_valid) in api_keys {
        ...
        assert_eq!(
            key_result.is_valid, should_be_valid,
    ...
    // Test rate limiting
    for i in 0..15 {
        let rate_limit_result = security_validator
            .check_rate_limit("test_user", "api_call")
            .await;
        if i >= 10 {
            // Assume 10 requests per minute limit
            assert!(
                !rate_limit_result.allowed,
                "Should be rate limited after threshold, request: {}",
                i
            );
        }
    }
...
async fn test_session_security(security_validator: &Arc<SecurityValidator>) {
    ...
    let integrity_result = security_validator
        .validate_session_integrity(&session_id)
        .await;
    assert!(integrity_result.is_valid, "Session should be valid");

    // Test session data protection
    session_manager
        .add_message_to_session(&session_id, "Sensitive data")
        .await
        .expect("Failed to add message");

    let protection_result = security_validator
        .validate_session_data_protection(&session_id)
        .await;
    assert!(
        protection_result.is_encrypted,
        "Session data should be encrypted"
    );

    // Test session expiration
    security_validator
        .set_session_timeout(&session_id, Duration::from_secs(1))
        .await;

    tokio::time::sleep(Duration::from_secs(2)).await;

    let expired_result = security_validator
        .validate_session_integrity(&session_id)
        .await;
...
async fn run_regression_detection_tests(
    ...
    let current_metrics = performance_monitor
        .get_current_metrics()
        .await
    ...
    let security_baseline = security_validator
        .get_security_baseline()
        .await
        .expect("Failed to get security baseline");

    let current_security = security_validator
    ...
    assert!(
        current_security.vulnerability_count <= security_baseline.vulnerability_count,
        "Security regression: new vulnerabilities detected"
    );

    // Ensure no degradation in security score
    assert!(
        current_security.security_score >= security_baseline.security_score * 0.95,
...
async fn generate_validation_report(
    ...
    // Security section
    report.push_str("\n## Security Status\n\n");
    let security_status = security_validator
        .get_current_security_status()
        .await
...
async fn test_system_stability_under_load() {
    ...
    assert!(
        operation_count > 100,
        "Should have completed sufficient operations"
    );
...
async fn test_graceful_degradation() {
    ...
    assert!(
        degraded_performance < Duration::from_secs(30),
    ...
    assert!(
        security_status.security_score > 50.0,
    ...
    assert!(
        recovered_performance < degraded_performance,
