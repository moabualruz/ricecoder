...
use ricecoder_performance::{PerformanceMonitor, PerformanceMetrics, PerformanceValidator};
...
use ricecoder_providers::{ProviderManager, providers::OpenAiProvider};
...
async fn test_full_system_integration_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    provider_manager.register_provider(Box::new(OpenAiProvider::new("test-key".to_string()))).await?;
    ...
    for i in 1..=10 {
        ...
        session_manager.set_session_name(session_id.clone(), format!("perf-test-session-{}", i)).await?;
    ...
    for session_id in session_ids.clone() {
        ...
        let handle = tokio::spawn(async move {
            for i in 1..=5 {
                ...
                session_mgr.add_message(session_id.clone(), message).await.unwrap();
            }
    ...
    assert!(validation_result.is_within_baselines, "Performance should be within baselines");
...
async fn test_end_to_end_workflow_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let project = Project::new(
        ...
        workspace_root.join("perf-test").to_string_lossy().to_string(),
    )?;
    std::fs::create_dir_all(&project.root_path)?;

    // Benchmark end-to-end workflow
    runner.benchmark("full_workflow", || async {
        // 1. Create session
        let session_id = session_manager.create_session("openai", "gpt-4").await?;
        ...
        for i in 1..=5 {
            ...
            session_manager.add_message(session_id.clone(), message).await?;
        }
    ...
    assert!(results.overall_score >= 0.7, "Workflow performance should be acceptable");

    // Validate against performance baselines
    let validator = PerformanceValidator::new();
    let validation = validator.validate_workflow_performance(&results).await?;
    assert!(validation.passed, "Workflow should pass performance validation");
...
async fn test_concurrent_session_performance_under_load() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..num_sessions {
        ...
        let handle = tokio::spawn(async move {
            ...
            session_mgr.set_session_name(session_id.clone(), format!("load-test-{}", i)).await.unwrap();
    ...
    for session_id in session_ids {
        let loaded = session_manager.load_session(&session_id).await?;
        assert!(loaded.is_some(), "Session {} should be loadable", session_id);
    ...
    let operations_per_second = (num_sessions * num_operations_per_session) as f64 / concurrent_time.as_secs_f64();
    assert!(operations_per_second >= 50.0, "Should handle at least 50 operations per second");

    // Check memory usage (mock implementation)
    let memory_metrics = performance_monitor.get_memory_usage().await?;
    assert!(memory_metrics.current_mb < 500.0, "Memory usage should be reasonable");
...
async fn test_provider_integration_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let provider_manager = ProviderManager::new();
    let performance_monitor = ricecoder_providers::performance_monitor::ProviderPerformanceMonitor::new();

    // Register providers
    provider_manager.register_provider(Box::new(OpenAiProvider::new("test-key".to_string()))).await?;
    provider_manager.register_provider(Box::new(ricecoder_providers::providers::AnthropicProvider::new("test-key".to_string()))).await?;
    ...
    for request in test_requests {
        ...
        performance_monitor.record_request(&request, response_time, true).await?;
    }
    ...
    assert!(avg_response_time < Duration::from_millis(200), "Average response time should be < 200ms");

    // Check provider performance summary
    let summary = performance_monitor.generate_summary().await?;
    assert!(summary.average_response_time < Duration::from_millis(150), "Provider performance should be good");
...
async fn test_mcp_integration_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(discovery_time < Duration::from_millis(50), "Tool discovery should be fast");
    ...
    for tool in tools {
        ...
        // Mock tool execution
        let result = tool_registry.execute_tool(&tool.name, serde_json::json!({})).await;
    ...
    assert!(avg_execution_time < Duration::from_millis(100), "Tool execution should be reasonably fast");
    ...
    assert!(pool_acquire_time < Duration::from_millis(200), "Connection pool should be fast");
...
async fn test_system_scalability_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=num_projects {
        ...
        for j in 1..=num_files_per_project {
            ...
            let content = format!("// Scalability test file {} in project {}\nfn test_{}() {{}}\n", j, i, j);
            std::fs::write(file_path, content)?;
        }
    }
    ...
    for i in 0..num_projects {
        let session_id = session_manager.create_session("openai", "gpt-4").await?;
        session_manager.set_session_name(session_id.clone(), format!("scale-session-{}", i)).await?;
        session_ids.push(session_id);
    }
    ...
    assert!(files_per_second >= 100.0, "Should create at least 100 files per second");
    assert!(projects_per_second >= 5.0, "Should scan at least 5 projects per second");
    assert!(sessions_per_second >= 10.0, "Should create at least 10 sessions per second");

    // Test memory usage under load
    let performance_monitor = PerformanceMonitor::new();
    let memory_usage = performance_monitor.get_memory_usage().await?;
    assert!(memory_usage.current_mb < 1000.0, "Memory usage should remain reasonable under load");
...
async fn test_performance_regression_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let regression_alerts = regression_detector.detect_regressions(&current_metrics).await?;

    // Should detect initialization time regression
    assert!(!regression_alerts.is_empty(), "Should detect performance regression");

    let init_regression = regression_alerts.iter().find(|a| a.metric == "initialization_time_ms");
    assert!(init_regression.is_some(), "Should detect initialization time regression");

    let alert = init_regression.unwrap();
    assert!(alert.percentage_change > 15.0, "Regression should be significant");
    ...
    let threshold_validation = performance_monitor.validate_thresholds(&current_metrics, &thresholds).await?;
