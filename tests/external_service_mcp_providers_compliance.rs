...
use ricecoder_mcp::{MCPClient, ToolRegistry, client::MCPClientConfig};
use ricecoder_providers::{ProviderManager, Provider, providers::{OpenAiProvider, AnthropicProvider}};
use ricecoder_security::{compliance::ComplianceManager, audit::AuditLogger};
...
async fn test_mcp_provider_integration_with_compliance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    provider_manager.register_provider(Box::new(openai_provider)).await?;
    provider_manager.register_provider(Box::new(anthropic_provider)).await?;
    ...
    assert!(compliance_result.is_compliant, "MCP config should be compliant");

    // Test provider compliance
    let providers = provider_manager.list_providers().await?;
    for provider in providers {
        let provider_compliance = compliance_manager.validate_provider_config(&provider).await?;
        assert!(provider_compliance.is_compliant, "Provider {} should be compliant", provider.id);
    }
...
#[tokio::test]
async fn test_mcp_tool_execution_with_permission_enforcement() -> Result<(), Box<dyn std::error::Error>> {
    let temp_dir = tempdir()?;
    let storage_path = temp_dir.path().to_path_buf();
    ...
    let read_result = tool_registry.check_permissions("safe_read_file", &["user"]).await?;
    assert!(read_result.allowed, "Read tool should be allowed");

    let write_result = tool_registry.check_permissions("dangerous_write_file", &["user"]).await?;
    ...
    assert!(!audit_entries.is_empty(), "Should have audit entries for permission checks");

    Ok(())
}

#[tokio::test]
async fn test_provider_mcp_integration_with_security_headers() -> Result<(), Box<dyn std::error::Error>> {
    let temp_dir = tempdir()?;
    ...
    assert!(validation_result.is_valid, "Security headers should be valid");
    ...
    assert!(!redacted.contains("abc123"), "User token should be redacted");
    ...
    assert!(compliance_result.is_compliant, "Redacted data should be compliant");

    Ok(())
}

#[tokio::test]
async fn test_mcp_provider_fallback_with_compliance_monitoring() -> Result<(), Box<dyn std::error::Error>> {
    let temp_dir = tempdir()?;
    let storage_path = temp_dir.path().to_path_buf();
    ...
    provider_manager.set_fallback_strategy(vec![
        "openai".to_string(),
        "anthropic".to_string(),
    ]).await?;
    ...
    let compliance_result = compliance_manager.validate_provider_fallback(&provider_manager).await?;
    assert!(compliance_result.is_compliant, "Provider fallback should be compliant");

    // Test audit logging of fallback events
    audit_logger.log_event("provider_fallback", "openai->anthropic", "test-user").await?;

    let audit_entries = audit_logger.get_entries("test-user").await?;
    assert!(audit_entries.iter().any(|e| e.action == "provider_fallback"),
...
async fn test_mcp_enterprise_compliance_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(mcp_report.is_compliant, "MCP should be enterprise compliant");

    // Test provider evaluation for enterprise use
    let evaluation_result = provider_compliance.evaluate_enterprise_readiness().await?;
    assert!(evaluation_result.overall_score >= 0.8, "Providers should be enterprise-ready");

    // Test data residency compliance
    let data_residency_check = ricecoder_security::compliance::DataPortability::new();
    let residency_result = data_residency_check.validate_data_location("us-west-2").await?;
    assert!(residency_result.compliant, "Data residency should be compliant");
    ...
    assert!(privacy_report.privacy_score >= 0.9, "Privacy score should be high");
...
async fn test_mcp_provider_rate_limiting_with_compliance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let compliance_manager = ComplianceManager::new();
    let rate_limit_compliance = compliance_manager.validate_rate_limiting(&rate_limiter).await?;
    assert!(rate_limit_compliance.is_compliant, "Rate limiting should be compliant");
